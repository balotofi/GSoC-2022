...
| WEEK  | ACTIVITY  |  
|---|---|
| WEEK 1- 2  | [Jun 13-25] - Because there was no assigned tasks or structured meeting times within the first two weeks, I spent that time creating the functions from scratch. I got the functions from the ESA SMP handbook which is a guide that we were using to structure the platform as discussed in Chapter 2. Initially, that was what I was doing - defining function and creating classes based off of those generic instructions. A commit I made during this time was [removed test lines in orbit propagator file](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/9/diffs?commit_id=cbcc3b11790eff9dbf1a3b8388c011981d0f7c56). In order to calculate for the the movement of an object in space over a period of 6 hours, I had previously copied code (from a source I can't recall now) to generate the propagation and see if it would run and how I could replicate parts of that code in my own `orbit_propagator` function. When I didn't understand how to use it, I removed it from the file. | 
| WEEK 2-4  | [Jun 27- Jul 9] - I had a meeting with Juan where we had a mini code review to make sure that I was doing the correct thing. We pivoted to the Orbit Propagator and he introduced me to PoliAstro (which we decided would be the best package to use for the propagation). I had trouble running it in VSCode due to a virtual environment installation issue. Artur’s feedback on my code was that I didn't need to implement the ICD word for word, that I just needed to do things that helped complete the defined tasks. He then assigned my first task to create basic coordinate systems classes so I first had to find out the different types of coordinate systems. A commit I made during this time was [deleted previous functions and inserted updatePost as stated in icd](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/9/diffs?commit_id=a6c3f263eacd6855292e1782d45c0a99814d12df). After the meeting with Juan I removed some redundant skeletal functions because we concluded that we didn't need those functionalities yet. I had implemented them because they were stated as part of the structure in the ICD. | 
| WEEK 4-6  | [Jul 11-23] - I was unaware how to implement it from scratch so Artur created it to work with the simulators architecture and assigned me to calculate the position vector of Earth as seen in the Sun coordinates system and that logic was to go into the `do_update` method of the derived coordinate system class. I reached out to a fellow contributor Hrishit who was working on the electrical module, so that he might explain the mapping from the design documents to Python for me. The commits I made during this time were in my [forked repo](https://gitlab.com/balotofi/python-libresim/-/commits/develop) but I deleted the branch when the code was deemed unuseful. A commit I made during this time was [updated field initialisations in orbit propagator and added positon printing...](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/9/diffs?commit_id=7a4c2b8e7957919bad08dc0963a0c1973607491a). Trying to figure out the architecture, I modified code from the example files into the `coord_sys.py` file and defined functions that I would later use to print desired values.| 

### Month 2-3 

|  WEEK | ACTIVITY  |  
|---|---|
| WEEK 6-8  | [Jul 25- Aug 6] - I got the position vectors updating but was getting some negative values and doubted their correctness. Juan suggested using SciPy’s transform rotation method to calculate the rotations. I had to do some research on what a quaternion was (the method of representation of the rotations). SciPy also has a method called `as_quat` that represents matrices, vectors, or Euler angle values as quaternions. I later started getting quantity conversion issues when subtracting positions from transformed vectors. It was around this time Artur suggested commit directly into the LibreSim codebase and gave me access to do so because of the issue of keeping up to date with the latest commits from develop in my fork. A commit I made during this time was [do update in earthcenecliptical configured using astropy geocentrictrueeclip... ](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/9/diffs?commit_id=17bdcaa587b85b24764b1c2ee712e25e3f966de3). To the `examples/coordinate_systems.py` file I added functionalty which imported from `coordinate_system.py` in the `src`, to the `print_fields` function so I could run the example and check if the code I writeen was outputting the desired values. | 
| WEEK 8-10  | [Aug 8-20] - I created a merge request because I had finally got rotations outputting values in a coordinate system and I wanted to do the same for other coordinate systems. The code was manually merged. The mid-term evaluations fell during this time. I had frequent hospital visits at the time and travelled cross-country for personal reasons. Made 3 merge requests during this time: [Draft: Coord sys method updates - do update method for earth fixed system](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/11), [Coord sys method updates](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/10), and [Draft: Position and Rotation outputs from do_update methods of EarthCentricEcliptical and EarthCentricEquatorial](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/9). Commits were made in [this](https://gitlab.com/librecube/prototypes/python-libresim/-/commits/orbital-coordsys?author=Husseinat%20Etti-Balogun) and [this](https://gitlab.com/librecube/prototypes/python-libresim/-/commits/orbital-celestbods?author=Husseinat%20Etti-Balogun) branch. A commit I made during this time was [completed rotation and position output in earth centric ecliptical coord sys](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/9/diffs?commit_id=32ad07b17fb2d1a16a0e59dd7f3934616388b52b). I imported `Rotation.align_vectors`, and `as_quat` from the SciPy package in order to calculate rotation vector of the `EarthCentricEcliptical` coordinate system and set the quaternion values. | 
| WEEK 10-12  | [Aug 22- Sept 3] - Artur did some architecture restructuring because it was understood that the way we had implemented the coordinate system was incorrect. My code that computed positions was compared to values from another package (SkyField) and the results were different so I had to correct it. Late August was when I was finally able to write code that computed similar values to SkyField for the position vectors with the appropriate coordinate systems. Artur then asked me to create a UML diagram for the module as he thought it would be helpful in understanding the concept of the architecture a little bit better. Made [Draft: Orbital coordsys](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/14) merge request. Commits were made in [this](https://gitlab.com/librecube/prototypes/python-libresim/-/commits/orbital-coordsys?author=Husseinat%20Etti-Balogun) and [this](https://gitlab.com/librecube/prototypes/python-libresim/-/commits/orbital-celestbods?author=Husseinat%20Etti-Balogun) branch. The last commit made in the `orbital-coordsys` branch was [added ecliptical frame to example](https://gitlab.com/librecube/prototypes/python-libresim/-/commit/8e52c856987f381755b1513a13da553571547ca7). Because Artur said the results that were being printed in my merge request were not accurate, in order to check the difference in values, I created a `load_earth.py` file that used only the SkyField API to print the ecliptic position coordinates of the Earth. Because I couldn't automatically generate the ephemerides file that was to be used using the `load` function, Artur sent it to me and I used the local file path loader function `Loader()` to load the `de421.bsp` file at the default `epoch_time` to print the results. It was then that I could see the difference in the values between my code and the more accurate SkyField code. |

### Month 4

|  WEEK | ACTIVITY  |  
|---|---|
| WEEK 12-14  | [Sept 5-17] - Since the coordinate system layout was now a service where coordinate systems could only be registered, I needed to replicate my previous code in individual planet files so that they would be the ones printing the positions and rotations. Again, I thought I had to build it from scratch so I created the celestial body IDs using an enumerations class before being reminded that the package AstroPy already had necessary planetary data. Commits were made in [this](https://gitlab.com/librecube/prototypes/python-libresim/-/commits/orbital-celestbods?author=Husseinat%20Etti-Balogun) and [this](https://gitlab.com/librecube/prototypes/python-libresim/-/commits/orbital-celestials?author=Husseinat%20Etti-Balogun) branch. A commit made to the `orbital-celestbods` branch was [attempting dynamic celest body example](https://gitlab.com/librecube/prototypes/python-libresim/-/commit/af0065d496097f2bd207fdc01360ff6a8aa687cb). I wanted to make the selection/ position value printing of coordinate systems more dynamic so that the user could select from the terminal which coordinate system they wanted to use. I didn't work out as hoped. | 
| WEEK 14-16  | [Sept 19- Oct 1] - I had a meeting with Artur where he sent me some links to code that was not part of the simulator but still within the LibreCube system that I could use to calculate the rotations. I got the position vectors running with a minor bug and then I fell ill and took an excuse to take time off to recuperate. I was unable to download JPL ephemerides files because of my location so they were sent them to me through WeTransfer. The week after, I got rotations running as well and created a merge request [Earth and Jupiter do update rotations](https://gitlab.com/librecube/prototypes/python-libresim/-/merge_requests/17) that was reviewed by Juan. An early commit I made during this time was [earth update now prints values](https://gitlab.com/librecube/prototypes/python-libresim/-/commit/af41ed2e69e748dc9c6ac7da2bb06eaafdcf150e). Within the `earth.py` file in the `do_update` function, I used the Astropy package to find, update and print the position coordinates of the Earth at the the simulator's `epoch_time`. | 
